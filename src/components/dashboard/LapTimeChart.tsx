...
const LapTimeChart = ({ data }: { data: LapRecord[] }) => {
  ...
  // Build stint ranges for shading
  ...
  // Compute rolling average (window=5) for each driver
  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-foreground">Performance Evolution</CardTitle>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
          <span>— thin: raw lap time</span>
          <span>— bold: 5-lap rolling avg</span>
          <span className="inline-block w-3 h-3 rounded bg-muted/20 border border-border" /> stint zone
        </div>
      </CardContent>
    </Card>
  );
