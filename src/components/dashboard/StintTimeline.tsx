  ...
  return (
    <Card className="bg-card border-border">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-semibold text-foreground">Stint Timeline</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={stintData} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
            ...
            <Tooltip
              ...
              formatter={(value: number) => [formatLapTime(value), 'Avg Pace']}
        ...
        {pitStints.length > 0 && (
          <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
            <span className="w-3 h-3 rounded-full bg-[hsl(340,65%,60%)]" /> Pit event in stint
          </div>
        )}
      </CardContent>
    </Card>
